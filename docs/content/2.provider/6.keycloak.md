...
For Keycloak you have to provide at least the `baseUrl`, `clientId` and `redirectUri` properties. Confidential clients also require `clientSecret`. The `baseUrl` is used to dynamically create the `authorizationUrl`, `tokenUrl`, `logoutUrl` and `userInfoUrl`.
Please include the realm you want to use in the `baseUrl` (e.g. `https://<keycloak-url>/realms/<realm>`).
If you don't want to use the post logout redirect feature of Keycloak, set `logoutUrl` to `undefined` or `''`.
