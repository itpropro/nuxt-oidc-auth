...
The `/api/_auth/sso` event stream accepts only a valid, unexpired sealed session cookie for a configured provider whose current session has `singleSignOut: true`. Refreshable sessions must also satisfy the configured missing-persistent-session policy. Invalid or ineligible requests receive `401` before an event stream is opened.
