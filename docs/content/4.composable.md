...
| params | Additional parameters to include in the login request. Parameters must be listed in `allowedClientAuthParameters` in the provider configuration, except `callbackRedirectUrl` which is handled by the module and can be used to return to a local path after callback. | `Record<string, string>` | No |
